...
def arcade_as_skeleton(image):
    ...
    # Extrai o esqueleto do obje


    image_skeleton = morphology.skeletonize(image)
    # pylab.imsave("./skeleton.png", image)

    return image_skeleton, image
...
def find_dental_arcade(image, threshold):
    ...
    projection_slice = image.max(0) >= 2000
    #skeleton_image, image_morpho = arcade_as_skeleton(projection_slice)

    skeleton_image, image_morpho = arcade_as_skeleton(best_slice)
    return skeleton_image, best_slice, best_slice_number, image_morpho
