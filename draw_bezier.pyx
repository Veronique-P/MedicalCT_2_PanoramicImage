...
# @cython.cdivision(True)
# @cython.wraparound(False)
def identity_fc(image_t[:,:,:] a, np.float64_t[:, :, :] b):
  print('ok', 2**16)
  return a
...
def planify_curves(image_t[:, :, :] image, np.float64_t[:, :, :] curves):
    ...
    # TODO: the np.int32 should be image_t instead
    output = np.zeros(shape=(ncurves, dz, npoints), dtype=np.int32) #dtype=np.int16)

    for z in prange(output.shape[0], nogil=True):  # loop over each curve
        for y in range(output.shape[1]):  # planar coordinates
            for x in range(output.shape[2]):  # points along the curves
